...
namespace HappreeTool.Surfers
{
    ...
    {
        private readonly ConcurrentDictionary<string, ChromeDriver> _driverCache =
            new ConcurrentDictionary<string, ChromeDriver>();

        /// <summary>
        /// 创建或附加到一个 Chrome 浏览器实例，并通过 ChromeDriver 控制它。如果指定的远程调试端口已经被使用，则附加到该实例。
        /// </summary>
        /// <param name="chromeLocation">chrome exe的路径</param>
        /// <param name="driverDir">chrome driver的路径</param>
        /// <param name="debugHost">chrome远程调试端口，默认为 127.0.0.1</param>
        /// <param name="debugPort">chrome远程调试端口，默认为 9222</param>
        ...
        public ChromeDriver GetOrCreateChromeDriver(string chromeLocation,
                                                    string driverDir,
        ...
        {
            string debuggerAddress = $"{debugHost}:{debugPort}";
            ...
            // 检查指定的端口是否被使用
            if (IsPortInUse(debugHost, debugPort))
            {
